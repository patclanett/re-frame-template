...
* [`src/first_project/`](src/first_project/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/first_project/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `first-project` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/first_project/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns first-project.example
  (:require [first-project.config :as config])
